import { Controller, Delete, Get, Post, Put, Param, Body, HttpCode } from "@nestjs/common/decorators"; 
...
export class ReportController {
  ...
  @Post()
  createReport(
    @Body() { amount, source }: CreateReportDto, @Param("type", new ParseEnumPipe(ReportType)) type: string): ReportResponseDto {
      const reportType = type === "income" ? ReportType.INCOME : ReportType.EXPENSE;
  ...
  @Put(":id")
  updateReport(
    @Param("type", new ParseEnumPipe(ReportType)) type: string, @Param("id", ParseUUIDPipe) id: string, @Body() body: UpdateReportDto
  ): ReportResponseDto {
    const reportType = type === "income" ? ReportType.INCOME : ReportType.EXPENSE;
